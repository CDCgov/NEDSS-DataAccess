...
REM Where "-P ***", replace with the related SQL user password.
REM Change the file path of the *.sql and *.log files as required for your configuration.

REM sqlcmd -S SQL_server\instance -d RDB -U nbs_rdb -P *** -Q "execute msdb.dbo.sp_start_job @job_name='ExecuteSSIS'" > D:\wildfly-10.0.0.Final\nedssdomain\Nedss\report\log\SSIS.log
REM sqlcmd -S SQL_server\instance -d RDB -U nbs_rdb -P *** -i D:\wildfly-10.0.0.Final\nedssdomain\Nedss\BatchFiles\SSIS_SP_BATCH_WAIT_COMPLETE.sql
...
sqlcmd -S cdc-nbs-alabama-rds-mssql.czya31goozkz.us-east-1.rds.amazonaws.com -U nbs_rdb -P *** -d RDB -Q "Exec RDB.dbo.sp_SLD_ALL" > D:\wildfly-10.0.0.Final\nedssdomain\Nedss\report\log\SSIS.log
