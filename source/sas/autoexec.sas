...
filename dyndmpgm  "&SAS_REPORT_HOME/dw/dynamicDm/src";
...
%if &SAS_REPORT_DBTYPE=ORACLE %then %do;
  libname nbs_ods oracle user=nbs_ods password=pwd path=nbsdb schema=nbs_odse ACCESS=READONLY;
  libname nbs_rdb oracle user=nbs_rdb password=pwd path=nbs_rdb ACCESS=READONLY;
  libname nbs_srt oracle user=nbs_ods password=pwd path=nbsdb schema=nbs_srte ACCESS=READONLY;
  libname nbs_msg oracle user=NBS_MSGOUT password=pwd path=nbs_msg  schema=NBS_MSGOUTE;
  %end;
%else %do;
  libname nbs_ods ODBC DSN=nedss1 UID=nbs_ods PASSWORD=pwd ACCESS=READONLY;  
  libname nbs_rdb ODBC DSN=nbs_rdb UID=nbs_rdb PASSWORD=pwd ACCESS=READONLY;
  libname nbs_srt ODBC DSN=nbs_srt UID=nbs_ods PASSWORD=pwd ACCESS=READONLY;
  libname nbs_msg ODBC DSN=nbs_msg UID=nbs_ods PASSWORD=pwd;
  %end;
%mend rptlib;
...
%if &SAS_REPORT_DBTYPE=ORACLE %then
	%do;
  libname nbs_ods oracle user=nbs_ods PASSWORD=pwd path=nbsdb schema=nbs_odse;
  libname nbs_srt oracle user=nbs_ods PASSWORD=pwd path=nbsdb schema=nbs_srte ACCESS=READONLY;
  libname nbs_rdb oracle user=nbs_rdb PASSWORD=pwd path=nbs_rdb  schema=nbs_rdb  DIRECT_EXE=DELETE;
%end;
%else
	%do;
  libname nbs_ods ODBC DSN=nedss1 UID=nbs_ods PASSWORD=pwd  ignore_read_only_columns=yes; 
  libname nbs_srt ODBC DSN=nbs_srt UID=nbs_ods PASSWORD=pwd ACCESS=READONLY;
  libname nbs_rdb ODBC DSN=nbs_rdb UID=nbs_rdb PASSWORD=pwd  DIRECT_EXE=DELETE;
...
%else
%do;
  libname nbs_ods ODBC DSN=nedss1 UID=nbs_ods PASSWORD=pwd; 
  libname nbs_srt ODBC DSN=nbs_srt UID=nbs_ods PASSWORD=pwd ACCESS=READONLY;
  libname nbs_msg ODBC DSN=nbs_msg UID=nbs_ods PASSWORD=pwd;
%end;	
%mend msglib;
