...
%if %SYSFUNC(LIBREF(nbs_ods)) NE 0 or
	...
	%SYSFUNC(LIBREF(nbs_rdb)) NE 0 or
	%SYSFUNC(LIBREF(nbs_cdc)) NE 0 or
	%SYSFUNC(LIBREF(nbs_rdbm)) NE 0
%then %do;
%etllib;
...
Proc SQL;
 Create table rdbdatam.imrdbmapping as
...
Proc SQL;
 Create table rdbdatam.code_val_general as
  select a.cd, b.code as CODE_VAL, 
   ...
   from rdbdatam.Codeset a, nbs_srt.Code_Value_General b
   where a.code_set_nm = b.code_set_nm;
Quit;

data rdbdatam.code_val_general;
set rdbdatam.code_val_general;
	code_key = _n_;
run;
...
Data _null_;
if 0 then set rdbdatam.codeset nobs=n_codeset;
if 0 then set rdbdatam.code_val_general nobs=n_cvg;
...
%if &n_codeset ~= 0 and &n_cvg ~= 0 %then %do;
	...
	%dbdelete(codeset);
	%dbload(codeset, rdbdatam.codeset);
	%dbload(code_val_general, rdbdatam.code_val_general);
	%end;
%else
