...
FROM rdb_modern.DBO.ACTIVITY_LOG_MASTER 
WHERE 
ACTIVITY_LOG_MASTER_UID= (SELECT MAX(ACTIVITY_LOG_MASTER_UID) FROM rdb_modern..ACTIVITY_LOG_MASTER );
) by sql;
disconnect from sql; 
Quit;
...
Proc SQL;
 Create table PHCroot_INIT as 
  Select phc.public_health_case_uid                    label='public_health_case_uid',
   ...
   From nbs_cdc.Public_health_case phc 
	...
	left join nbs_cdc.Act_relationship act 
   ...
   /* Ravi -- where phc.LAST_CHG_TIME> (SELECT MAX(ACTIVITY_LOG_MASTER_LAST.START_DATE) FROM  ACTIVITY_LOG_MASTER_LAST)*/
   /* addded new where clause for process logic - ravi */
   where phc.cdc_status<>1
...
Proc SQL;
 Create table rdbdata.PHCrootobs_NONINC as
  Select phc.*,
		 ...
		 phc.cd as condition_cd                        label='condition_cd'
   From  PHCroot_NONINC phc left outer join nbs_cdc.observation obs
...
Proc SQL;
 Create table PHCchild_NONINC as
  Select phc.public_health_case_uid,
   ...
   From nbs_cdc.Act_relationship act, rdbdata.PHCrootobs_NONINC phc
...
Proc SQL;
 Create table rdbdata.PHCchildobs_NONINC as
  Select phc.*,
   ...
   From nbs_cdc.observation obs, PHCchild_NONINC phc 
...
Proc SQL;
 Create table rdbdata.PHCrootobs as
  Select phc.*,
  		 obs.cd as root_obs_cd                        label='root_obs_cd'
   From  PHCRoot phc left outer join nbs_cdc.observation obs
    on obs.observation_uid = phc.root_obs_uid
    Order By public_health_case_uid, root_obs_uid;        /* sort required for merge with fact tables */
Quit;
...
Proc SQL;
 Create table PHCchild as
  Select phc.public_health_case_uid,
		 ...
		 phc.investigation_form_cd,
         act.source_act_uid as child_obs_uid           label='child_obs_uid'
   From nbs_cdc.Act_relationship act, rdbdata.PHCrootobs phc
    Where act.target_act_uid = phc.root_obs_uid
	  and act.type_cd = 'InvFrmQ'
	  and act.source_class_cd = 'OBS' 
	  and act.target_class_cd = 'OBS'
...
Proc SQL;
 Create table rdbdata.PHCchildobs as
  Select phc.*,
         obs.cd as child_obs_cd                        label='child_obs_cd'
   From nbs_cdc.observation obs, PHCchild phc 
    Where obs.observation_uid = phc.child_obs_uid;
Quit;
