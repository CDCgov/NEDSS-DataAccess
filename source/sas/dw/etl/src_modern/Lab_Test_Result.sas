...
create table Lab_Test_Result1 as select
	...
	*/
	left join nbs_cdc.observation 	as obs
...
FROM	Result_And_R_Result			as rslt
		LEFT JOIN nbs_cdc.obs_value_txt	as otxt
		...
		*/
		...
		LEFT JOIN  nbs_cdc.obs_value_numeric	as onum
		ON rslt.lab_test_uid = onum.observation_uid

		LEFT JOIN nbs_cdc.obs_value_coded		as code
		ON rslt.lab_test_uid = code.observation_uid

		LEFT JOIN nbs_cdc.obs_value_date		as date
