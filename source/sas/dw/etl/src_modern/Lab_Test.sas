...
proc sql;
create table s_edx_document as select EDX_Document_uid, act_uid, add_time from nbs_cdc.EDX_Document, nbs_rdb.s_updated_lab where  EDX_Document.act_uid=s_updated_lab.observation_Uid order by add_time desc;
quit;
...
observation_uid
from nbs_cdc.participation, UPDATED_OBSERVATION_LIST obs 
where 
...
/* from updated_observation_List left outer join nbs_cdc.observation as obs */
...
create table lab_test_mat_init as select 
	...
	inner join nbs_cdc.material	as mat
...
rsn.reason_desc_txt,
rsn.reason_cd 
FROM updated_observation_List obs LEFT JOIN  nbs_cdc.observation_reason	as rsn
...
create table lab_test_oth as select 
	...
	*/
	...
	from OBS_REASON obs left join 	nbs_cdc.act_id  as ai
		on obs.observation_uid = ai.ACT_UID
	  	and ai.type_cd='FN'
		/*and ai.act_id_seq=2*/
	left join nbs_cdc.observation_interp	as oin
		on obs.observation_uid = oin.observation_uid
		and oin.INTERPRETATION_CD ~=' '
	/*left join nbs_cdc.observation_reason	as rsn
		on obs.observation_uid= rsn.observation_uid*/
...
from LabReportMorb l, nbs_cdc.act_relationship ar, nbs_cdc.observation o 
...
from 	Lab_Test1 as tst,
		/*R_Result_to_R_Order*/
		nbs_cdc.act_relationship	as act
...
from 	R_Result_to_R_Order	as tst
	/*R_Order to Order */
	left join	nbs_cdc.act_relationship as act2
	...
	left join	nbs_cdc.act_relationship as act3
	...
	left join	nbs_cdc.act_relationship as act4
...
from 	Lab_Test1 as tst,
		...
		nbs_cdc.act_relationship	as act,
		/*Result to Order */
		nbs_cdc.act_relationship	as act2
...
from nbs_cdc.observation as obs, lab_test1 as tst
...
/* 		nbs_cdc.act_relationship 	as ar1, */	
/* 		nbs_cdc.observation			as obs, */	
/* 		nbs_cdc.act_relationship 	as ar2, */	
/* 		nbs_cdc.observation			as lab214, */	
/* 		nbs_cdc.obs_value_txt 				as ovt */	
