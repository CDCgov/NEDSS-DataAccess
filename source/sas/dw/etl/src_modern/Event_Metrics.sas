...
FROM NOT_PROG_JURI_CVG  N
  	/* add user name */
	LEFT outer join nbs_cdc.user_profile as up1
		    on n.add_user_id = up1.nedss_entry_id
	/* last change user name */
	LEFT outer join nbs_cdc.user_profile as up2
...
INSERT INTO nbs_rdb.EVENT_METRIC 
  ...
  FROM nbs_cdc.CT_CONTACT
  INNER JOIN nbs_cdc.PERSON ON 
