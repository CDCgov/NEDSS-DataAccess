...
RUN;
PROC SQL;
	UPDATE ACTIVITY_LOG_MASTER_LAST SET ODSE_COUNT = (SELECT COUNT(*) FROM nbs_cdc.Interview);
...
CREATE TABLE  
	S_INTERVIEW_UIDS AS  
SELECT 
	INTERVIEW_UID LENGTH =8 AS INTERVIEW_UID 'INTERVIEW_UID', LAST_CHG_TIME FROM nbs_cdc.INTERVIEW
...
CREATE TABLE 
	...
	FROM S_INTERVIEW_UIDS inner join nbs_cdc.INTERVIEW ON
...
SELECT DISTINCT NBS_ANSWER.ANSWER_TXT, NBS_ANSWER.NBS_ANSWER_UID, NBS_ANSWER.ACT_UID AS ACT_UID 'ACT_UID' FROM nbs_cdc.NBS_ANSWER INNER JOIN
