...
select 	act.target_act_uid		as intervention_uid label='Intervention uid',
		...
		obs.cd,
		obc.code
from 	nbs_cdc.act_relationship as act,
		nbs_cdc.observation		as obs,
		nbs_cdc.obs_value_coded	as obc
where 	
...
from 	nbs_cdc.act_relationship 	as act,
		nbs_cdc.observation			as obs,
		nbs_cdc.obs_value_numeric	as obc
where 	
		act.source_act_uid  = obs.observation_uid
		and act.type_cd = 'VAC105'
		and act.source_class_cd='OBS'
...
from 	nbs_cdc.Intervention 		as vac
	...
	left join	nbs_cdc.material				as mat
		on par4.subject_entity_uid = mat.material_uid

	/* vaccine manufacturer*/
	left join	nbs_cdc.role					as rol1
	...
	left join	nbs_cdc.act_relationship		as act
