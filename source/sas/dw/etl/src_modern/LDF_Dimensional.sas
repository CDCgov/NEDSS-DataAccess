...
proc transpose data=  ldf_data_translated out=  ldf_data_translated_rows;
    by   business_object_uid ldf_uid code_set_nm label_txt data_source data_type Field_size;
...
run;
data ldf_data_translated_rows; 
length phc_cd_updated $200;
set ldf_data_translated_rows; by business_object_uid; 
retain phc_cd_updated;
if  first.business_object_uid then phc_cd_updated =phc_cd; 
else phc_cd=phc_cd_updated;
run;  
proc sql;
	create table 
...
run;
proc sort data= ldf_base_country_translated; by business_object_uid ldf_uid; 
run;
data  ldf_base_country_translated; 
length x $4000; 
length col1 $4000; 
	set  ldf_base_country_translated; by business_object_uid ldf_uid; 
	retain x; 
	if  first.ldf_uid then x=' '; x=catx(' | ',x,col1); 
	if last.ldf_uid; 
	if lengthn(x)>0 then col1=x;
run; 
/*proc sort data= ldf_base_country_translated; by business_object_uid cdc_national_id; 
run;
data  ldf_base_country_translated; 
length x $4000; 
length col1 $4000; 
set  ldf_base_country_translated; by business_object_uid cdc_national_id; 
retain x; 
if  first.cdc_national_id then x=' '; x=catx(' | ',x,col1); 
if last.cdc_national_id; 
if lengthn(x)>0 then col1=x;
run;
*/
...
ldf_metadata 
as select  
ldf_uid, label_txt, cdc_national_id, condition_cd, class_cd,custom_subform_metadata_uid,LDF_PAGE_SET, data_type, Field_size
from ldf_meta_data
order by cdc_national_id, ldf_uid, label_txt;
quit;
proc sql;
create table ldf_metadata_n
as select a.cdc_national_id,label_txt as ldf_label 'ldf_label', a.ldf_uid,a.condition_cd,a.class_cd,custom_subform_metadata_uid,LDF_PAGE_SET
...
datamart_column_nm = tranwrd(trim(datamart_column_nm), "/", "_");
put datamart_column_nm;
datamart_column_nm = tranwrd(trim(datamart_column_nm), ",", "_");
put datamart_column_nm;
datamart_column_nm = tranwrd(trim(datamart_column_nm), "@", "_");
put datamart_column_nm;
...
if class_cd='STATE' 
	then  datamart_column_nm = compress("L_"||''||trim(ldfchar)||''|| trim(datamart_column_nm) ) ;
ELSE if (lencdcnatid>1 and (lencdcnatid+ len) and lenCSid>1 ) 
	then datamart_column_nm= compress("C_"||''||cdc_national_id||''||datamart_column_nm );
else if class_cd='CDC' 
	then datamart_column_nm= compress(trim("C_"||''||trim((ldfchar))||''||datamart_column_nm));
...
%include etlpgm (LDF_Hepatitis_Datamart.sas);
%include etlpgm (LDF_Foodborne_Datamart.sas);
%include etlpgm (LDF_Bmird_Datamart.sas);
%include etlpgm (LDF_Tetanus_Datamart.sas);
%include etlpgm (LDF_Mumps_Datamart.sas);
