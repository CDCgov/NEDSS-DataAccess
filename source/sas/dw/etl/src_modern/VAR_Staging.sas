...
select public_health_case_UID LENGTH =8 AS VAR_PAM_UID 'VAR_PAM_UID' from nbs_cdc.public_health_case where cd in (
...
JOIN nbs_cdc.NBS_Case_Answer PA ON PQ.NBS_QUESTION_UID =PA.NBS_QUESTION_UID
...
FROM nbs_cdc.NBS_ACT_ENTITY, S_VAR_PAM  where 
...
UPDATE ACTIVITY_LOG_DETAIL SET SOURCE_ROW_COUNT=(SELECT COUNT(*) FROM nbs_cdc.NBS_ACT_ENTITY where last_chg_time>(select start_date from ACTIVITY_LOG_MASTER_LAST)),
END_DATE=DATETIME(),
...
%DBLOAD (ACTIVITY_LOG_DETAIL, ACTIVITY_LOG_DETAIL);
proc sql;
create table case_entity as select * from nbs_cdc.NBS_ACT_ENTITY 
where ACT_UID in (select public_health_case_UID from nbs_cdc.public_health_case where cd in (
select  condition_cd from nbs_srt.condition_code where investigation_form_cd = 'INV_FORM_VAR'
))
