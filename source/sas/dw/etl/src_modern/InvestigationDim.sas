...
on 
	...
	left join nbs_cdc.participation	as par2
...
PROC SQL;
    ...
    create index INVESTIGATION_KEY on INVESTIGATION_E_KEYHOLDER(INVESTIGATION_KEY);
	create index INVESTIGATION_KEY on INVESTIGATION_N_KEYHOLDER(INVESTIGATION_KEY);
	create index INVESTIGATION_KEY on INVESTIGATION_N_ALL_KEYHOLDER(INVESTIGATION_KEY);
...
PROC SORT DATA=D_INVESTIGATION_E NODUPKEY; BY INVESTIGATION_KEY;RUN;
%DBLOAD (D_INVESTIGATION_E, D_INVESTIGATION_E);
/*%DBLOAD (INVESTIGATION_E_KEYHOLDER, INVESTIGATION_E_KEYHOLDER);*/
...
Data _Null_;
	...
	put '*****The Start CONFIRMATION_METHOD_E create table*********';
run;
*/
PROC SQL;
CREATE TABLE CONFIRMATION_METHOD_E AS SELECT CONFIRMATION_METHOD_KEY FROM nbs_rdb.CONFIRMATION_METHOD_GROUP WHERE INVESTIGATION_KEY IN (SELECT INVESTIGATION_KEY FROM INVESTIGATION_E_KEYHOLDER);
QUIT;
Data _Null_;
	format Time Datetime20.;
	Time=datetime();
	put '	End Time:   ' Time   Datetime20.;
	put '*****The End CONFIRMATION_METHOD_E create table*********';
run; 
/*
DELETE FROM nbs_rdb.CONFIRMATION_METHOD 
	WHERE CONFIRMATION_METHOD_KEY IN 
	(SELECT CONFIRMATION_METHOD_KEY FROM nbs_rdb.CONFIRMATION_METHOD_GROUP WHERE INVESTIGATION_KEY IN (SELECT INVESTIGATION_KEY FROM INVESTIGATION_E_KEYHOLDER));
*/


PROC SQL;
SELECT count(INVESTIGATION_KEY) into :cnt FROM nbs_rdb.INVESTIGATION_E_KEYHOLDER;
QUIT;
...
Data _Null_;
	...
	Time=datetime();
	put '	Start Time:   ' Time   Datetime20.;
	put '*****The Start DELETE FROM nbs_rdb.CONFIRMATION_METHOD_GROUP*********';
...
/* RAVI Start 
PROC SQL;
DELETE FROM nbs_rdb.CONFIRMATION_METHOD_GROUP WHERE INVESTIGATION_KEY IN (SELECT INVESTIGATION_KEY FROM INVESTIGATION_E_KEYHOLDER);
DELETE FROM nbs_rdb.CONFIRMATION_METHOD_GROUP cmg inner join INVESTIGATION_E_KEYHOLDER i on cmg.INVESTIGATION_KEY = i.INVESTIGATION_KEY;
QUIT;
Ravi end */
...
Data _Null_;
	...
	Time=datetime();
	put '	end Time:   ' Time   Datetime20.;
	put '*****The end DELETE FROM nbs_rdb.CONFIRMATION_METHOD_GROUP*********';
run;		
Data _Null_;
	format Time Datetime20.;
	Time=datetime();
	put '	Start Time:   ' Time   Datetime20.;
	put '*****The Start DELETE FROM nbs_rdb.CONFIRMATION_METHOD*********';
...
/*if &cnt >= 1 then do;*/
	...
	/*
	DELETE FROM nbs_rdb.L_CONFIRMATION_METHOD WHERE INVESTIGATION_KEY IN (SELECT INVESTIGATION_KEY FROM nbs_rdb.INVESTIGATION_E_KEYHOLDER);
	*/
	
	/* RAVI START
	DELETE FROM nbs_rdb.L_CONFIRMATION_METHOD lcm
	join INVESTIGATION_E_KEYHOLDER i on i.INVESTIGATION_KEY = lcm.INVESTIGATION_KEY; RAVI END*/
	QUIT;
/*end;*/
...
Data _Null_;
	...
	Time=datetime();
	put '	end Time:   ' Time   Datetime20.;
	put '*****The end DELETE FROM nbs_rdb.CONFIRMATION_METHOD*********';
run;
...
from nbs_cdc.Confirmation_Method Confirmation_method
inner join nbs_cdc.Public_health_case PHC 
...
(
    ...
    FROM nbs_cdc.Confirmation_Method Confirmation_method
         INNER JOIN nbs_cdc.Public_health_case PHC ON Confirmation_method.public_health_case_uid = PHC.public_health_case_uid
		 and phc.cdc_status<>1 /*COMMENT RAVI Start -- added this - COMMENT - RAVI END*/
       /* COMMENT: Ravi start - and phc.LAST_CHG_TIME> (SELECT MAX(ACTIVITY_LOG_MASTER_LAST.START_DATE) FROM  ACTIVITY_LOG_MASTER_LAST) :COMMENT: Ravi end*/
    GROUP BY Confirmation_method.public_health_case_uid
) a
INNER JOIN nbs_cdc.Confirmation_Method Confirmation_method ON a.public_health_case_uid = Confirmation_method.public_health_case_uid
GROUP BY Confirmation_method.public_health_case_uid, 
         confirmation_method_cd, 
         a.confirmation_method_time;
quit;
