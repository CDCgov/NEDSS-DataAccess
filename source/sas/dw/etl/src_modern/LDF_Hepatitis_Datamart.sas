PROC SQL;
DROP TABLE nbs_rdb.LDF_HEPATITIS;

CREATE TABLE BASE_HEPATITIS AS 
SELECT * FROM LDF_DIMENSIONAL_DATA WHERE  PHC_CD IN(SELECT CONDITION_CD FROM 
		nbs_rdb.LDF_DATAMART_TABLE_REF WHERE DATAMART_NAME = 'LDF_HEPATITIS');
...
CREATE TABLE ALL_HEPATITIS AS 
	...
	FULL OUTER JOIN LINKED_HEPATITIS A 
...
RUN;
PROC SORT DATA=ALL_HEPATITIS NODUPKEY OUT=ALL_HEPATITIS; BY INVESTIGATION_KEY DATAMART_COLUMN_NM; RUN;
PROC SQL;
CREATE TABLE ALL_HEPATITIS_SHORT_COL AS
SELECT * FROM ALL_HEPATITIS WHERE data_type IN ('CV', 'ST');

CREATE TABLE ALL_HEPATITIS_TA AS
SELECT * FROM ALL_HEPATITIS WHERE data_type IN ('LIST_ST');

QUIT;
DATA ALL_HEPATITIS_TA;
SET ALL_HEPATITIS_TA;
LENGTH ANSWERCOL $2000;
ANSWERCOL=COL1;
DROP COL1;
RUN;
DATA ALL_HEPATITIS_SHORT_COL;
SET ALL_HEPATITIS_SHORT_COL;
LENGTH ANSWERCOL $200;
ANSWERCOL=COL1;
DROP COL1;
RUN;
PROC TRANSPOSE DATA=  ALL_HEPATITIS_TA  OUT=  HEPATITIS_TA;
	...
	VAR ANSWERCOL;
RUN;
PROC TRANSPOSE DATA=  ALL_HEPATITIS_SHORT_COL  OUT=  HEPATITIS_SHORT_COL;
    BY   INVESTIGATION_KEY;
   	COPY INVESTIGATION_KEY INVESTIGATION_LOCAL_ID PROGRAM_JURISDICTION_OID PATIENT_KEY PATIENT_LOCAL_ID DISEASE_NAME DISEASE_CD;
    ID DATAMART_COLUMN_NM;
	VAR ANSWERCOL;
RUN;
PROC SQL;
DELETE * FROM HEPATITIS_SHORT_COL WHERE _NAME_ IS NULL;
DELETE * FROM HEPATITIS_TA WHERE _NAME_ IS NULL;
QUIT;
DATA HEPATITIS;
...
RUN;
PROC SQL;
DELETE * FROM HEPATITIS WHERE INVESTIGATION_KEY IS NULL;
QUIT;
PROC SQL;
CREATE TABLE  nbs_rdb.LDF_HEPATITIS AS SELECT * FROM HEPATITIS;
QUIT;
