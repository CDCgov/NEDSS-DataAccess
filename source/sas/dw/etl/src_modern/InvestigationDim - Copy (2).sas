...
PROC SQL;
SELECT count(INVESTIGATION_KEY) into :cnt FROM INVESTIGATION_E_KEYHOLDER;
QUIT;
/*%put &sqlobs;*/
/*
%put "count of key = " &cnt;
run;
*/
/*
Data _Null_;
...
/*data _null_;
	if &cnt > 0 then call execute('%deleteCMG');
run;
*/
/*if &cnt >= 1 then do;	*/
	PROC SQL ;
...
*/
/*data _null_;
	if &cnt > 0 then call execute('%deleteCM');
run;
*/
/*if &cnt >= 1 then do;*/
	PROC SQL;
	DELETE FROM nbs_rdb.L_CONFIRMATION_METHOD WHERE INVESTIGATION_KEY IN (&s1) ; 
	/* RAVI START DELETE FROM nbs_rdb.L_CONFIRMATION_METHOD WHERE INVESTIGATION_KEY IN (SELECT INVESTIGATION_KEY FROM INVESTIGATION_E_KEYHOLDER); RAVI END*/
	/* RAVI START
