...
SELECT ANSWER_TXT,ACT_UID AS PAGE_CASE_UID 'PAGE_CASE_UID',ANSWER_GROUP_SEQ_NBR, PLACE_TYPE.* FROM nbs_cdc.NBS_CASE_ANSWER INNER JOIN PLACE_TYPE ON 
...
(NULL);
UPDATE DATA_CHECKER_MASTER_LAST SET COUNT= (SELECT COUNT(*)  FROM nbs_cdc.NBS_CASE_ANSWER WHERE
NBS_QUESTION_UID IN (
