...
PROC SQL;
	...
	FROM nbs_cdc.treatment AS rx1 
	INNER JOIN nbs_cdc.Treatment_administered AS rx2 
	ON rx1.treatment_uid = rx2.treatment_uid 

	LEFT JOIN nbs_cdc.act_relationship AS act1 
	...
	LEFT JOIN nbs_cdc.uvw_treatment_patient_keys as  viewPatientKeys
...
PROC SQL;
	...
	SELECT DISTINCT morb.Morb_rpt_key,  t.treatment_uid FROM nbs_rdb.Morbidity_report AS morb, nbs_cdc.act_relationship AS act, treatment_all1 AS t
