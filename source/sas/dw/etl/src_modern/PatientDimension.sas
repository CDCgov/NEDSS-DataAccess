...
FROM  PATIENT_REV_UID INNER JOIN nbs_cdc.PERSON_RACE
...
PROC SQL;
CREATE TABLE S_PERSON_NAME AS SELECT  PERSON_NAME.PERSON_UID AS  PATIENT_UID ' PATIENT_UID' ,PERSON_NAME_SEQ,  FIRST_NM,LAST_NM,MIDDLE_NM,NM_SUFFIX, NM_USE_CD FROM  PATIENT_REV_UID INNER JOIN nbs_cdc.PERSON_NAME
...
FROM PATIENT_REV_UID LEFT OUTER JOIN nbs_cdc.ENTITY_LOCATOR_PARTICIPATION
...
FROM PATIENT_REV_UID INNER JOIN nbs_cdc.ENTITY_LOCATOR_PARTICIPATION
	ON PATIENT_REV_UID.PATIENT_UID= ENTITY_LOCATOR_PARTICIPATION.ENTITY_UID
...
FROM PATIENT_REV_UID LEFT OUTER JOIN nbs_cdc.ENTITY_ID
