...
/*  Format /* multiple_birth_ind $1.
         ...
         nm_suffix $18.
		 marital_status_cd $1.;  */
  Set nbs_cdc.person (keep=local_id birth_time curr_sex_cd as_of_date_sex birth_gender_cd multiple_birth_ind
...
Data degree;
  Set nbs_cdc.person_name (keep=person_uid nm_degree nm_use_cd person_name_seq );
  Where nm_use_cd = 'L';
Run;
...
Data ODS_person_name;
  Set nbs_cdc.person_name (keep=first_nm middle_nm last_nm nm_prefix nm_suffix nm_use_cd);
...
Proc SQL;
 Create Table personnames as
  Select a.person_key,
   ...
   From rdbdata.person1 a LEFT JOIN nbs_cdc.person_name b
     On a.person_uid = b.person_uid;
Quit;
...
Proc SQL;
 Create Table personraces as
  Select a.person_key, b.race_cd, b.as_of_date, b.RACE_CATEGORY_CD, b.record_status_cd
   From rdbdata.person1 a LEFT JOIN nbs_cdc.person_race b
     On a.person_uid = b.person_uid;
Quit;
...
Proc SQL;
 Create Table personethnics as
  Select a.person_key, b.ethnic_group_cd, a.as_of_date_ethnicity as person_ethnicity_as_of_dt, b.record_status_cd
   From rdbdata.person1 a LEFT JOIN nbs_cdc.person_ethnic_group b
     On a.person_uid = b.person_uid;
Quit;
...
Proc SQL;
 Create Table personrelationships1 as
  Select p.person_key, 
		 ...
		 r.scoping_class_cd,
		 r.record_status_cd
   From rdbdata.person1 p LEFT JOIN nbs_cdc.role r
...
Proc SQL;
 Create Table rdbdata.person_id as
  Select a.person_key, 
		 ...
		 b.record_status_cd
   From rdbdata.person1 a LEFT JOIN nbs_cdc.entity_id b
...
/*Get the SSN from nbs_cdc.Entity_Id table as opposed...currently the application is not 
saving the snn in nbs_cdc.Person.snn
*/
proc sql;
create table rdbdata.person as
select p.*, e.root_extension_txt as patient_ssn 
from rdbdata.person1 as p 
left join nbs_cdc.entity_id as e 
