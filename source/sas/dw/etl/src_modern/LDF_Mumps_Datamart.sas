PROC SQL;
DROP TABLE nbs_rdb.LDF_MUMPS;

CREATE TABLE BASE_MUMPS AS 
SELECT * FROM LDF_DIMENSIONAL_DATA WHERE  PHC_CD IN(SELECT CONDITION_CD FROM 
		nbs_rdb.LDF_DATAMART_TABLE_REF WHERE DATAMART_NAME = 'LDF_MUMPS');
...
CREATE TABLE ALL_MUMPS AS 
	...
	FULL OUTER JOIN LINKED_MUMPS A 
...
RUN;
PROC SORT DATA=ALL_MUMPS NODUPKEY OUT=ALL_MUMPS; BY INVESTIGATION_KEY DATAMART_COLUMN_NM; RUN;
PROC SQL;
CREATE TABLE ALL_MUMPS_SHORT_COL AS
SELECT * FROM ALL_MUMPS WHERE data_type IN ('CV', 'ST');

CREATE TABLE ALL_MUMPS_TA AS
SELECT * FROM ALL_MUMPS WHERE data_type IN ('LIST_ST');
...
DROP COL1;
RUN;
DATA ALL_MUMPS_SHORT_COL;
SET ALL_MUMPS_SHORT_COL;
LENGTH ANSWERCOL $200;
ANSWERCOL=COL1;
DROP COL1;
RUN;
PROC TRANSPOSE DATA=  ALL_MUMPS_TA  OUT=  MUMPS_TA;
	...
	VAR ANSWERCOL;
RUN;
PROC TRANSPOSE DATA=  ALL_MUMPS_SHORT_COL  OUT=  MUMPS_SHORT_COL;
    BY   INVESTIGATION_KEY;
   	COPY INVESTIGATION_KEY INVESTIGATION_LOCAL_ID PROGRAM_JURISDICTION_OID PATIENT_KEY PATIENT_LOCAL_ID DISEASE_NAME DISEASE_CD;
    ID DATAMART_COLUMN_NM;
	VAR ANSWERCOL;
RUN;
PROC SQL;
DELETE * FROM MUMPS_SHORT_COL WHERE _NAME_ IS NULL;
DELETE * FROM MUMPS_TA WHERE _NAME_ IS NULL;
QUIT;
DATA MUMPS;
...
RUN;
PROC SQL;
DELETE * FROM MUMPS WHERE INVESTIGATION_KEY IS NULL;
QUIT;
PROC SQL;
CREATE TABLE  nbs_rdb.LDF_MUMPS AS SELECT * FROM MUMPS;
QUIT;
