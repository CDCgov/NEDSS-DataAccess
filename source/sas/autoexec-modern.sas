...
filename dyndmpgm  "&SAS_REPORT_HOME/dw/dynamicDm/src_modern";
...
%if &SAS_REPORT_DBTYPE=ORACLE %then %do;
  libname nbs_ods oracle user=nbs_ods password=ooo path=nbsdb schema=nbs_odse ACCESS=READONLY;
  libname nbs_cdc ODBC DSN=nbs_changedata UID=nbs_ods password=ooo;  
  libname nbs_rdb oracle user=nbs_rdb_modern password=ooo path=nbs_rdb ACCESS=READONLY;
  libname nbs_srt oracle user=nbs_ods password=ooo path=nbsdb schema=nbs_srte ACCESS=READONLY;
  libname nbs_msg oracle user=NBS_MSGOUT password=MSG path=nbs_msg  schema=NBS_MSGOUTE;
  %end;
%else %do;
  libname nbs_ods ODBC DSN=nedss1 UID=nbs_ods password=ooo ACCESS=READONLY;  
  libname nbs_cdc ODBC DSN=nbs_changedata UID=nbs_ods password=ooo;  
  libname nbs_rdb ODBC DSN=nbs_rdb_modern UID=nbs_rdb password=ooo ACCESS=READONLY;
  libname nbs_srt ODBC DSN=nbs_srt UID=nbs_ods password=ooo ACCESS=READONLY;
  libname nbs_msg ODBC DSN=nbs_msg UID=nbs_ods password=ooo;
  %end;
%mend rptlib;
...
%if &SAS_REPORT_DBTYPE=ORACLE %then
	%do;
  libname nbs_ods oracle user=nbs_ods password=ooo path=nbsdb schema=nbs_odse;
  libname nbs_cdc ODBC DSN=nbs_changedata UID=nbs_ods password=ooo  ignore_read_only_columns=yes;   
  libname nbs_srt oracle user=nbs_ods password=ooo path=nbsdb schema=nbs_srte ACCESS=READONLY;
  libname nbs_rdb oracle user=nbs_rdb password=ooo path=nbs_rdb  schema=nbs_rdb  DIRECT_EXE=DELETE;
%end;
%else
	%do;
  libname nbs_ods ODBC DSN=nedss1 UID=nbs_ods password=ooo  ignore_read_only_columns=yes; 
  libname nbs_cdc ODBC DSN=nbs_changedata UID=nbs_ods password=ooo  ignore_read_only_columns=yes; 
  libname nbs_srt ODBC DSN=nbs_srt UID=nbs_ods password=ooo ACCESS=READONLY;
  libname nbs_rdb ODBC DSN=nbs_rdb_modern UID=nbs_rdb password=ooo  DIRECT_EXE=DELETE;
...
%else
%do;
  libname nbs_ods ODBC DSN=nedss1 UID=nbs_ods password=ooo; 
  libname nbs_srt ODBC DSN=nbs_srt UID=nbs_ods password=ooo ACCESS=READONLY;
  libname nbs_msg ODBC DSN=nbs_msg UID=nbs_ods password=ooo;
%end;	
%mend msglib;
