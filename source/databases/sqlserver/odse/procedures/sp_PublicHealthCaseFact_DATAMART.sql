...
CREATE PROCEDURE [dbo].[sp_PublicHealthCaseFact_DATAMART]
AS
BEGIN
	...
	BEGIN TRANSACTION;
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA;
		...
		INTO NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA
	...
	IF OBJECT_ID('NBS_ODSE.DBO.TEMP_UPDATE_NEW_PATIENT') IS NOT NULL
	DROP TABLE NBS_ODSE.DBO.TEMP_UPDATE_NEW_PATIENT;
		...
		INTO NBS_ODSE.DBO.TEMP_UPDATE_NEW_PATIENT
...
IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCPATIENTINFO') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCPATIENTINFO;
		SELECT
			PER.*
		...
		INTO NBS_ODSE.DBO.TEMP_PHCPATIENTINFO
		FROM NBS_ODSE.DBO.TEMP_UPDATE_NEW_PATIENT PER WITH (NOLOCK)
		INNER JOIN NBS_ODSE.DBO.PARTICIPATION PAR WITH (NOLOCK) ON PER.PERSON_UID = PAR.SUBJECT_ENTITY_UID
		INNER JOIN NBS_ODSE.DBO.Public_health_case Public_health_case WITH (NOLOCK) ON PAR.act_uid = Public_health_case.public_health_case_uid
		...
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA A ON A.CODE=PER.ETHNIC_GROUP_IND
		...
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA AGE_UNIT WITH (NOLOCK) ON PER.age_reported_unit_cd = AGE_UNIT.CODE
		...
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA BIRTH_GENDER WITH (NOLOCK) ON PER.birth_gender_cd = BIRTH_GENDER.CODE
		...
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA CURR_SEX WITH (NOLOCK) ON PER.curr_sex_cd = CURR_SEX.CODE
		...
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA CVG2 WITH (NOLOCK) ON PER.MARITAL_STATUS_CD = CVG2.CODE
		...
		DELETE
		FROM NBS_ODSE.DBO.TEMP_PHCPATIENTINFO
		WHERE RN > 1

		UPDATE NBS_ODSE.DBO.TEMP_PHCPATIENTINFO
		...
		FROM NBS_ODSE.DBO.TEMP_PHCPATIENTINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCPATIENTINFO.ETHNIC_GROUP_IND = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM = 'P_ETHN_GRP'
			AND NBS_ODSE.DBO.TEMP_PHCPATIENTINFO.INVESTIGATION_FORM_CD NOT LIKE 'PG_%' or INVESTIGATION_FORM_CD is null

		
		UPDATE NBS_ODSE.DBO.TEMP_PHCPATIENTINFO
		SET MARITAL_STATUS_DESC_TXT = CODE_VALUE_GENERAL.CODE_SHORT_DESC_TXT
		FROM NBS_ODSE.DBO.TEMP_PHCPATIENTINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCPATIENTINFO.MARITAL_STATUS_CD = CODE_VALUE_GENERAL.CODE
		...
		FROM NBS_ODSE.DBO.TEMP_PHCPATIENTINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCPATIENTINFO.birth_gender_cd = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM = 'SEX'
		...
		FROM NBS_ODSE.DBO.TEMP_PHCPATIENTINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCPATIENTINFO.age_reported_unit_cd = CODE_VALUE_GENERAL.CODE
		...
		FROM NBS_ODSE.DBO.TEMP_PHCPATIENTINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCPATIENTINFO.curr_sex_cd = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM = 'SEX'
			AND NBS_ODSE.DBO.TEMP_PHCPATIENTINFO.INVESTIGATION_FORM_CD NOT LIKE 'PG_%' or INVESTIGATION_FORM_CD is null
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCINFO_INIT') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCINFO_INIT;
			
		SELECT OBSERVATION2.CD, TEMP_PHCPATIENTINFO.PUBLIC_HEALTH_CASE_UID,OBS_VALUE_DATE.FROM_TIME 
			INTO NBS_ODSE.DBO.TEMP_PHCINFO_INIT	
			FROM NBS_ODSE.DBO.TEMP_PHCPATIENTINFO WITH (NOLOCK)
			INNER JOIN NBS_ODSE.DBO.ACT_RELATIONSHIP WITH (NOLOCK) ON ACT_RELATIONSHIP.TARGET_ACT_UID=TEMP_PHCPATIENTINFO.PUBLIC_HEALTH_CASE_UID
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCINFO_LEGACY_HOSP') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCINFO_LEGACY_HOSP;
			
	SELECT OBSERVATION2.CD, TEMP_PHCPATIENTINFO.PUBLIC_HEALTH_CASE_UID,OBS_VALUE_CODED.CODE
			INTO NBS_ODSE.DBO.TEMP_PHCINFO_LEGACY_HOSP	
			FROM NBS_ODSE.DBO.TEMP_PHCPATIENTINFO WITH (NOLOCK)
			INNER JOIN NBS_ODSE.DBO.ACT_RELATIONSHIP WITH (NOLOCK) ON ACT_RELATIONSHIP.TARGET_ACT_UID=TEMP_PHCPATIENTINFO.PUBLIC_HEALTH_CASE_UID
			INNER JOIN NBS_ODSE.DBO.ACT_RELATIONSHIP ACT_RELATIONSHIP2 WITH (NOLOCK) ON ACT_RELATIONSHIP2.TARGET_ACT_UID=ACT_RELATIONSHIP.SOURCE_ACT_UID
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCINFO1') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCINFO1;
		...
		SELECT PHC.PUBLIC_HEALTH_CASE_UID
			...
			,SUBSTRING(PHC.MMWR_WEEK, 1, 4) AS MMWR_WEEK
			,SUBSTRING(PHC.MMWR_YEAR, 1, 4) AS MMWR_YEAR
			,PHC.OUTBREAK_IND
			,PHC.OUTBREAK_FROM_TIME
			,PHC.OUTBREAK_TO_TIME
			,PHC.OUTBREAK_NAME
			,PHC.OUTCOME_CD
			,SUBSTRING(PHC.PAT_AGE_AT_ONSET, 1, 4) AS PAT_AGE_AT_ONSET
			...
			,PHC.TXT AS PHCTXT
			,FOOD_HANDLER_IND_CD
			,hospitalized_ind_cd AS HOSPITALIZED_IND
			,DAY_CARE_IND_CD
			,PREGNANT_IND_CD
			,CONDITION_CODE.INVESTIGATION_FORM_CD
			...
			,NOTIF_LAST_CHG_TIME
			,TEMP_PHCINFO_LEGACY_HOSP.CODE AS LEGACY_HOSP_IND
		INTO NBS_ODSE.DBO.TEMP_PHCINFO1
		FROM NBS_ODSE.DBO.PUBLIC_HEALTH_CASE PHC WITH (NOLOCK)
		INNER JOIN NBS_ODSE.DBO.TEMP_PHCPATIENTINFO ON PHC.PUBLIC_HEALTH_CASE_UID = NBS_ODSE.DBO.TEMP_PHCPATIENTINFO.ACT_UID
		...
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_PHCINFO_INIT TEMP_PHCINFO_INITA ON  PHC.PUBLIC_HEALTH_CASE_UID = TEMP_PHCINFO_INITA.PUBLIC_HEALTH_CASE_UID
		AND TEMP_PHCINFO_INITA.CD='INV132'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_PHCINFO_INIT TEMP_PHCINFO_INITB ON  PHC.PUBLIC_HEALTH_CASE_UID = TEMP_PHCINFO_INITB.PUBLIC_HEALTH_CASE_UID
		AND TEMP_PHCINFO_INITB.CD='INV133'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_PHCINFO_LEGACY_HOSP  ON  PHC.PUBLIC_HEALTH_CASE_UID = TEMP_PHCINFO_LEGACY_HOSP.PUBLIC_HEALTH_CASE_UID
		AND TEMP_PHCINFO_LEGACY_HOSP.CD='INV128';
		SELECT @ROWCOUNT_NO = @@ROWCOUNT;

		PRINT @ROWCOUNT_NO;

		UPDATE NBS_ODSE.DBO.TEMP_PHCINFO1
		...
		ALTER TABLE NBS_ODSE.DBO.TEMP_PHCINFO1 DROP column LEGACY_HSPTL_DISCHARGE_DT, LEGACY_HSPTL_ADMISSION_DT;
		INSERT INTO [rdb].[dbo].[job_flow_log] (
			batch_id
			,[Dataflow_Name]
			,[package_Name]
		...
		WHERE public_health_case_uid IN (
				...
				FROM NBS_ODSE.DBO.TEMP_PHCINFO1
				);
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCINFO2') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCINFO2;

		SELECT A.*
			,NBS_CASE_ANSWER.ANSWER_TXT AS THERAPY_DATE
		INTO NBS_ODSE.DBO.TEMP_PHCINFO2
		FROM NBS_ODSE.DBO.TEMP_PHCINFO1 A WITH (NOLOCK)
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCINFO3') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCINFO3;

		SELECT A.*
			,AID.ROOT_EXTENSION_TXT AS STATE_CASE_ID
		INTO NBS_ODSE.DBO.TEMP_PHCINFO3
		FROM NBS_ODSE.DBO.TEMP_PHCINFO2 A WITH (NOLOCK)
		...
		UPDATE NBS_ODSE.DBO.TEMP_PHCINFO3
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCINFO') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCINFO;
		...
		INTO NBS_ODSE.DBO.TEMP_PHCINFO
		FROM NBS_ODSE.DBO.TEMP_PHCINFO3
		LEFT OUTER JOIN NBS_SRTE.DBO.CONDITION_CODE A WITH (NOLOCK) ON A.CONDITION_CD = NBS_ODSE.DBO.TEMP_PHCINFO3.PHC_CODE
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA B WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.DISEASE_IMPORTED_CD = B.CODE
			AND NBS_ODSE.DBO.TEMP_PHCINFO3.investigation_form_cd = B.investigation_form_cd
			AND B.DATA_LOCATION LIKE '%.DISEASE_IMPORTED_CD'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA C WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.DETECTION_METHOD_CD = C.CODE
			AND NBS_ODSE.DBO.TEMP_PHCINFO3.investigation_form_cd = C.investigation_form_cd
			AND C.DATA_LOCATION LIKE '%.DETECTION_METHOD_CD'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA D WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.RPT_SOURCE_CD = D.CODE
			AND NBS_ODSE.DBO.TEMP_PHCINFO3.investigation_form_cd = D.investigation_form_cd
			AND D.DATA_LOCATION LIKE '%.RPT_SOURCE_CD'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA E WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.HOSPITALIZED_IND = E.CODE
			AND NBS_ODSE.DBO.TEMP_PHCINFO3.investigation_form_cd = E.investigation_form_cd
			AND E.DATA_LOCATION LIKE '%.HOSPITALIZED_IND_CD'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA F WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.PREGNANT_IND_CD = F.CODE
			AND NBS_ODSE.DBO.TEMP_PHCINFO3.investigation_form_cd = F.investigation_form_cd
			AND F.DATA_LOCATION LIKE '%.PREGNANT_IND_CD'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA G WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.DAY_CARE_IND_CD = G.CODE
			AND NBS_ODSE.DBO.TEMP_PHCINFO3.investigation_form_cd = G.investigation_form_cd
			AND G.DATA_LOCATION LIKE '%.DAY_CARE_IND_CD'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA H WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.FOOD_HANDLER_IND_CD = H.CODE
			AND NBS_ODSE.DBO.TEMP_PHCINFO3.investigation_form_cd = H.investigation_form_cd
			AND H.DATA_LOCATION LIKE '%.FOOD_HANDLER_IND_CD'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA I WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.IMPORTED_COUNTRY_CODE = I.CODE
			AND NBS_ODSE.DBO.TEMP_PHCINFO3.investigation_form_cd = I.investigation_form_cd
			AND I.DATA_LOCATION LIKE '%.IMPORTED_COUNTRY_CD'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA J WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.IMPORTED_COUNTY_CODE = J.CODE
			AND NBS_ODSE.DBO.TEMP_PHCINFO3.investigation_form_cd = J.investigation_form_cd
			AND J.DATA_LOCATION LIKE '%.IMPORTED_COUNTY_CD'
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA K WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.IMPORTED_STATE_CODE = K.CODE
			AND NBS_ODSE.DBO.TEMP_PHCINFO3.INVESTIGATION_FORM_CD = K.INVESTIGATION_FORM_CD
			AND K.DATA_LOCATION LIKE '%.IMPORTED_STATE_CD'
		LEFT OUTER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL L WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.CASE_CLASS_CD = L.CODE
			AND L.CODE_SET_NM = 'PHC_CLASS'
		LEFT OUTER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL M WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.INVESTIGATION_STATUS_CD = M.CODE
			AND M.CODE_SET_NM = 'PHC_IN_STS'
		LEFT OUTER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL N WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.OUTCOME_CD = N.CODE
			AND N.CODE_SET_NM = 'YNU'
		LEFT OUTER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL O WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.PAT_AGE_AT_ONSET_UNIT_CD = O.CODE
			AND O.CODE_SET_NM = 'AGE_UNIT'
		LEFT OUTER JOIN NBS_SRTE.DBO.PROGRAM_AREA_CODE P WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.PROG_AREA_CD = P.PROG_AREA_CD
			AND P.CODE_SET_NM = 'S_PROGRA_C'
		LEFT OUTER JOIN NBS_SRTE.DBO.V_STATE_COUNTY_CODE_VALUE Q WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.RPT_CNTY_CD = Q.CODE
			AND Q.CODE_SET_NM = 'COUNTY_CCD'
		LEFT OUTER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL R WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO3.OUTBREAK_NAME = R.CODE
			AND R.CODE_SET_NM = 'OUTBREAK_NM'
			

		SELECT @ROWCOUNT_NO = @@ROWCOUNT;

		UPDATE NBS_ODSE.DBO.TEMP_PHCINFO
		...
		FROM NBS_ODSE.DBO.TEMP_PHCINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO.DISEASE_IMPORTED_CD = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM = 'PHC_IMPRT'
			AND NBS_ODSE.DBO.TEMP_PHCINFO.INVESTIGATION_FORM_CD NOT LIKE 'PG_%' or INVESTIGATION_FORM_CD is null

		UPDATE NBS_ODSE.DBO.TEMP_PHCINFO
		SET DETECTION_METHOD_DESC_TXT = CODE_VALUE_GENERAL.CODE_SHORT_DESC_TXT
		FROM NBS_ODSE.DBO.TEMP_PHCINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON NBS_ODSE.DBO.TEMP_PHCINFO.detection_method_cd = CODE_VALUE_GENERAL.CODE
		...
		FROM NBS_ODSE.DBO.TEMP_PHCINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON TEMP_PHCINFO.rpt_source_cd = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM = 'PHC_RPT_SRC_T'
			AND TEMP_PHCINFO.INVESTIGATION_FORM_CD NOT LIKE 'PG_%' or INVESTIGATION_FORM_CD is null

		UPDATE NBS_ODSE.DBO.TEMP_PHCINFO
		SET HOSPITALIZED = CODE_VALUE_GENERAL.CODE_SHORT_DESC_TXT
		FROM NBS_ODSE.DBO.TEMP_PHCINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON TEMP_PHCINFO.HOSPITALIZED_IND = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM = 'YNU'
			AND TEMP_PHCINFO.INVESTIGATION_FORM_CD NOT LIKE 'PG_%' or INVESTIGATION_FORM_CD is null
		...
		FROM NBS_ODSE.DBO.TEMP_PHCINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON TEMP_PHCINFO.pregnant_ind_cd = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM = 'YNU'
			AND TEMP_PHCINFO.INVESTIGATION_FORM_CD NOT LIKE 'PG_%' or INVESTIGATION_FORM_CD is null

		UPDATE NBS_ODSE.DBO.TEMP_PHCINFO
		SET DAY_CARE_IND_CD = CODE_VALUE_GENERAL.CODE_SHORT_DESC_TXT
		FROM NBS_ODSE..TEMP_PHCINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON TEMP_PHCINFO.CASE_DAY_CARE_IND_CD = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM = 'YNU'
			AND TEMP_PHCINFO.INVESTIGATION_FORM_CD NOT LIKE 'PG_%' or INVESTIGATION_FORM_CD is null
		...
		FROM NBS_ODSE..TEMP_PHCINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON TEMP_PHCINFO.FOOD_HANDLER_IND_CD = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM = 'YNU'
			AND TEMP_PHCINFO.INVESTIGATION_FORM_CD NOT LIKE 'PG_%' or INVESTIGATION_FORM_CD is null
		...
		SET DISEASE_IMPORTED_DESC_TXT = CODE_VALUE_GENERAL.CODE_SHORT_DESC_TXT
		FROM NBS_ODSE..TEMP_PHCINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON TEMP_PHCINFO.DISEASE_IMPORTED_CD = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM = 'PHC_IMPRT'
			AND TEMP_PHCINFO.INVESTIGATION_FORM_CD NOT LIKE 'PG_%' or INVESTIGATION_FORM_CD is null
		...
		FROM NBS_ODSE..TEMP_PHCINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON TEMP_PHCINFO.IMPORTED_COUNTRY_CODE = CODE_VALUE_GENERAL.CODE
		...
		FROM NBS_ODSE..TEMP_PHCINFO
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON TEMP_PHCINFO.IMPORTED_STATE_CD = CODE_VALUE_GENERAL.CODE
	...
	UPDATE NBS_ODSE.DBO.TEMP_PHCINFO
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCSUBJECT') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCSUBJECT;
		...
		SELECT --PUBLIC_HEALTH_CASE_UID,
			...
			,PROG_AREA_DESC_TXT
			,TEMP_PHCPATIENTINFO.*
		INTO NBS_ODSE.DBO.TEMP_PHCSUBJECT
		FROM NBS_ODSE.DBO.TEMP_PHCINFO WITH (NOLOCK)
		INNER JOIN NBS_ODSE.DBO.TEMP_PHCPATIENTINFO WITH (NOLOCK) ON TEMP_PHCINFO.PUBLIC_HEALTH_CASE_UID = TEMP_PHCPATIENTINFO.PUBLIC_HEALTH_CASE_UID
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_SUMMARYUID') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_SUMMARYUID;
		...
		INTO NBS_ODSE.DBO.TEMP_SUMMARYUID
		FROM NBS_ODSE.DBO.PUBLIC_HEALTH_CASE PHC WITH (NOLOCK)
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_CASECNT') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_CASECNT;
		...
		INTO NBS_ODSE.DBO.TEMP_CASECNT
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_SAS_LATEST_NOT') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_SAS_LATEST_NOT;
		...
		INTO NBS_ODSE.DBO.TEMP_SAS_LATEST_NOT
		...
		WHERE AR.SOURCE_ACT_UID = NF.NOTIFICATION_UID
			...
			OR  AR.TARGET_ACT_UID IN (SELECT PUBLIC_HEALTH_CASE_UID  FROM NBS_ODSE..TEMP_PHCSUBJECT WITH (NOLOCK)))
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_SAS_NOTIFICATION') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_SAS_NOTIFICATION;
		...
		INTO NBS_ODSE.DBO.TEMP_SAS_NOTIFICATION
		FROM NBS_ODSE.DBO.ACT_RELATIONSHIP AR WITH (NOLOCK)
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCFACT') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCFACT;

		SELECT TEMP_CASECNT.*
			,TARGET_CLASS_CD
			,TEMP_SAS_NOTIFICATION.SOURCE_ACT_UID
			,TEMP_SAS_NOTIFICATION.SOURCE_CLASS_CD
			,VERSION_CTRL_NBR
			,ADD_TIME
			,ADD_USER_ID
			,RPT_SENT_TIME
			...
			,NOTIFICATIONDATE
			INTO NBS_ODSE.DBO.TEMP_PHCFACT
		FROM NBS_ODSE.DBO.TEMP_CASECNT WITH (NOLOCK)
		LEFT JOIN NBS_ODSE.DBO.TEMP_SAS_NOTIFICATION WITH (NOLOCK) ON TEMP_SAS_NOTIFICATION.PUBLIC_HEALTH_CASE_UID = TEMP_CASECNT.PUBLIC_HEALTH_CASE_UID;
		SELECT @ROWCOUNT_NO = @@ROWCOUNT;
...
IF OBJECT_ID('NBS_ODSE.DBO.TEMP_MIN_MAX_NOTIFICATION') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_MIN_MAX_NOTIFICATION;
		...
		INTO NBS_ODSE.DBO.TEMP_MIN_MAX_NOTIFICATION
		FROM NBS_ODSE.DBO.TEMP_SAS_NOTIFICATION WITH (NOLOCK)
		GROUP BY PUBLIC_HEALTH_CASE_UID;

	UPDATE NBS_ODSE.DBO.TEMP_MIN_MAX_NOTIFICATION set NOTIFCREATEDCOUNT = NOTIFCREATEDCOUNT-1 where NOTIFCREATEDPENDINGSCOUNT>0 and NOTIFCREATEDCOUNT>0;
	UPDATE NBS_ODSE.DBO.TEMP_MIN_MAX_NOTIFICATION set NOTIFCREATEDCOUNT = 1 where NOTIFCREATEDPENDINGSCOUNT>0 and NOTIFCREATEDCOUNT=0 and NOTIFREJECTEDCOUNT=0;
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_SUB_CASE_NOTIF') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_SUB_CASE_NOTIF;

		SELECT DISTINCT TEMP_PHCSUBJECT.*
			,NOTIFCREATEDCOUNT
			,NOTIFSENTCOUNT
			,NOTIFICATIONDATE
			...
			,TEMP_SAS_LATEST_NOT.TXT AS NOTITXT
			,TEMP_SAS_LATEST_NOT.NOTIFICATION_LOCAL_ID
			,TEMP_SAS_LATEST_NOT.NOTIFCURRENTSTATE
			,ROWN = ROW_NUMBER() OVER (
				PARTITION BY TEMP_PHCSUBJECT.PUBLIC_HEALTH_CASE_UID ORDER BY TEMP_PHCSUBJECT.PUBLIC_HEALTH_CASE_UID
				)
		INTO NBS_ODSE.DBO.TEMP_SUB_CASE_NOTIF
		FROM NBS_ODSE.DBO.TEMP_PHCSUBJECT WITH (NOLOCK)
		LEFT JOIN NBS_ODSE.DBO.TEMP_MIN_MAX_NOTIFICATION WITH (NOLOCK) ON TEMP_MIN_MAX_NOTIFICATION.PUBLIC_HEALTH_CASE_UID = TEMP_PHCSUBJECT.PUBLIC_HEALTH_CASE_UID
		LEFT JOIN NBS_ODSE.DBO.TEMP_SAS_LATEST_NOT  WITH (NOLOCK) ON TEMP_SAS_LATEST_NOT.PUBLIC_HEALTH_CASE_UID = TEMP_PHCSUBJECT.PUBLIC_HEALTH_CASE_UID;

		SELECT @ROWCOUNT_NO = @@ROWCOUNT;
		
		DELETE
		FROM NBS_ODSE.DBO.TEMP_SUB_CASE_NOTIF
		WHERE ROWN > 1
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_CONFIRM') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_CONFIRM;

		SELECT CONFIRMATION_METHOD.PUBLIC_HEALTH_CASE_UID
			,A.CONFIRMATION_METHOD_TIME
			,CONFIRMATION_METHOD_CD
			,TEMP_INV_FORM_CODE_DATA.CODE_SHORT_DESC_TXT AS CONFIRMATION_METHOD_DESC_TXT
			,TEMP_INV_FORM_CODE_DATA.investigation_form_cd
		INTO NBS_ODSE.DBO.TEMP_CONFIRM
		FROM (
			SELECT CONFIRMATION_METHOD.PUBLIC_HEALTH_CASE_UID
		...
		INNER JOIN NBS_ODSE.DBO.TEMP_PHCINFO WITH (NOLOCK) ON TEMP_PHCINFO.PUBLIC_HEALTH_CASE_UID = A.PUBLIC_HEALTH_CASE_UID
		...
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_INV_FORM_CODE_DATA  WITH (NOLOCK) ON CONFIRMATION_METHOD.CONFIRMATION_METHOD_CD = TEMP_INV_FORM_CODE_DATA.CODE
		AND  TEMP_INV_FORM_CODE_DATA.investigation_form_cd = TEMP_PHCINFO.investigation_form_cd	
		AND TEMP_INV_FORM_CODE_DATA.CODE_SET_NM= 'PHC_CONF_M'
		GROUP BY CONFIRMATION_METHOD.PUBLIC_HEALTH_CASE_UID
			,CONFIRMATION_METHOD_CD
			,A.CONFIRMATION_METHOD_TIME
			,CONFIRMATION_METHOD_DESC_TXT
			,TEMP_INV_FORM_CODE_DATA.CODE_SHORT_DESC_TXT
			,TEMP_INV_FORM_CODE_DATA.investigation_form_cd;

		SELECT @ROWCOUNT_NO = @@ROWCOUNT;

			
		UPDATE NBS_ODSE.DBO.TEMP_CONFIRM
		SET CONFIRMATION_METHOD_DESC_TXT = CODE_VALUE_GENERAL.CODE_SHORT_DESC_TXT
		FROM NBS_ODSE.DBO.TEMP_CONFIRM
		INNER JOIN NBS_SRTE.DBO.CODE_VALUE_GENERAL WITH (NOLOCK) ON TEMP_CONFIRM.CONFIRMATION_METHOD_CD = CODE_VALUE_GENERAL.CODE
			AND CODE_SET_NM= 'PHC_CONF_M'
			where TEMP_CONFIRM.INVESTIGATION_FORM_CD NOT LIKE 'PG_%' or INVESTIGATION_FORM_CD is null
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_CONFIRM_CONCAT') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_CONFIRM_CONCAT;

		SELECT DISTINCT PUBLIC_HEALTH_CASE_UID
			,CONFIRMATION_METHOD_TIME = (
				SELECT TOP 1 CONFIRMATION_METHOD_TIME
				FROM NBS_ODSE.DBO.TEMP_CONFIRM WITH (NOLOCK)
		...
		INTO NBS_ODSE.DBO.TEMP_CONFIRM_CONCAT
		FROM NBS_ODSE.DBO.TEMP_CONFIRM AS T;
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_SUB_CASE_CONF_NOTIF') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_SUB_CASE_CONF_NOTIF;

		SELECT TEMP_SUB_CASE_NOTIF.*
			,CONFIRMATION_METHOD_TIME
			,CONFIRMATION_METHOD_CD
			,CONFIRMATION_METHOD_DESC_TXT
		INTO NBS_ODSE.DBO.TEMP_SUB_CASE_CONF_NOTIF
		FROM NBS_ODSE.DBO.TEMP_SUB_CASE_NOTIF WITH (NOLOCK)
		LEFT OUTER JOIN TEMP_CONFIRM_CONCAT WITH (NOLOCK) ON TEMP_SUB_CASE_NOTIF.PUBLIC_HEALTH_CASE_UID = TEMP_CONFIRM_CONCAT.PUBLIC_HEALTH_CASE_UID;
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCREPORTER') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCREPORTER;
		...
		INTO NBS_ODSE.DBO.TEMP_PHCREPORTER
		FROM NBS_ODSE.DBO.PARTICIPATION AS PAR WITH (NOLOCK)
		INNER JOIN NBS_ODSE.DBO.TEMP_PHCINFO on TEMP_PHCINFO.public_health_CASE_UID  =PAR.ACT_UID 	
		...
		FROM NBS_ODSE.DBO.PARTICIPATION AS PAR2 WITH (NOLOCK)
			INNER JOIN NBS_ODSE.DBO.TEMP_PHCINFO on TEMP_PHCINFO.public_health_CASE_UID  =PAR2.ACT_UID 	
		...
		WHERE  PAR2.RECORD_STATUS_CD = 'ACTIVE'
			...
			OR( TEMP_PHCINFO.LASTUPDATE >= @batch_start_time
				AND TEMP_PHCINFO.LASTUPDATE < @batch_end_time)
			OR (NOTIF_LAST_CHG_TIME >= @batch_start_time
				AND NOTIF_LAST_CHG_TIME < @batch_end_time)				
			ORDER BY ACT_UID
			,TYPE_CD;

		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_ENTITY') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_ENTITY;
		...
		SELECT ACT_UID
			...
			,ORGANIZATIONNAME = MAX(CASE 
					WHEN TYPE_CD = 'OrgAsReporterOfPHC'
						THEN NAME
					END)
		INTO NBS_ODSE.DBO.TEMP_ENTITY
		FROM NBS_ODSE.DBO.TEMP_PHCREPORTER WITH (NOLOCK)
		GROUP BY ACT_UID
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_CASE_ENTITY') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_CASE_ENTITY;

		SELECT TEMP_SUB_CASE_CONF_NOTIF.*
			,TEMP_ENTITY.PROVIDERNAME
			,TEMP_ENTITY.PROVIDERPHONE
			,TEMP_ENTITY.REPORTERNAME
			,TEMP_ENTITY.REPORTERPHONE
			,TEMP_ENTITY.INVESTIGATORNAME
			,TEMP_ENTITY.INVESTIGATORPHONE
			,TEMP_ENTITY.INVESTIGATORASSIGNEDDATE
			,TEMP_ENTITY.ORGANIZATIONNAME
		INTO NBS_ODSE.DBO.TEMP_CASE_ENTITY
		FROM NBS_ODSE.DBO.TEMP_SUB_CASE_CONF_NOTIF WITH (NOLOCK)
		LEFT OUTER JOIN NBS_ODSE.DBO.TEMP_ENTITY WITH (NOLOCK) ON TEMP_SUB_CASE_CONF_NOTIF.PUBLIC_HEALTH_CASE_UID = TEMP_ENTITY.ACT_UID;
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHCPERSONRACE_CONCAT') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHCPERSONRACE_CONCAT;

		SELECT DISTINCT PERSON_UID
			,RACE_CONCATENATED_DESC_TXT = STUFF((
					SELECT ', ' + RACE_DESC_TXT
					FROM NBS_ODSE.DBO.TEMP_PHCPERSONRACE WITH (NOLOCK)
		...
		INTO NBS_ODSE.DBO.TEMP_PHCPERSONRACE_CONCAT
		FROM NBS_ODSE.DBO.TEMP_PHCPERSONRACE AS T;
		...
		IF OBJECT_ID('NBS_ODSE.DBO.TEMP_PHC_FACT') IS NOT NULL
			DROP TABLE NBS_ODSE.DBO.TEMP_PHC_FACT;

		SELECT TEMP_CASE_ENTITY.*
			,RACE_CONCATENATED_DESC_TXT
			,RACE_CONCATENATED_TXT
		INTO NBS_ODSE.DBO.TEMP_PHC_FACT
		FROM NBS_ODSE.DBO.TEMP_CASE_ENTITY
		LEFT OUTER JOIN TEMP_PHCPERSONRACE_CONCAT WITH (NOLOCK) ON TEMP_CASE_ENTITY.PERSON_UID = TEMP_PHCPERSONRACE_CONCAT.PERSON_UID;

		----------------------
		ALTER TABLE NBS_ODSE.DBO.TEMP_PHC_FACT ADD EVENT_TYPE VARCHAR(20);

		ALTER TABLE NBS_ODSE.DBO.TEMP_PHC_FACT ADD EVENT_DATE DATETIME;

		ALTER TABLE NBS_ODSE.DBO.TEMP_PHC_FACT ADD REPORT_DATE DATETIME;

		ALTER TABLE NBS_ODSE.DBO.TEMP_PHC_FACT ADD MART_RECORD_CREATION_TIME DATETIME;
		...
		UPDATE NBS_ODSE.DBO.TEMP_PHC_FACT
		SET EVENT_TYPE = (
				CASE 
		...
		UPDATE NBS_ODSE.DBO.TEMP_PHC_FACT SET EVENT_DATE = THERAPY_DATE WHERE PHC_CODE ='10220' and THERAPY_DATE is not null ;
		UPDATE NBS_ODSE.DBO.TEMP_PHC_FACT SET EVENT_DATE = rpt_form_cmplt_time WHERE PHC_CODE ='10220' and THERAPY_DATE is null and rpt_form_cmplt_time is not null;
		UPDATE NBS_ODSE.DBO.TEMP_PHC_FACT SET EVENT_TYPE = 'RVCT_DTS' WHERE PHC_CODE ='10220' and THERAPY_DATE is not null ;
		UPDATE NBS_ODSE.DBO.TEMP_PHC_FACT SET EVENT_TYPE = 'RVCT_DR' WHERE PHC_CODE ='10220' and THERAPY_DATE is null and rpt_form_cmplt_time is not null;
		
		UPDATE NBS_ODSE.DBO.TEMP_PHC_FACT SET State_cd = NULL WHERE State_cd = LTRIM(RTRIM(''));
		UPDATE NBS_ODSE.DBO.TEMP_PHC_FACT SET phctxt = NULL WHERE phctxt = LTRIM(RTRIM(''));
		UPDATE NBS_ODSE.DBO.TEMP_PHC_FACT SET cntry_cd = NULL WHERE cntry_cd = LTRIM(RTRIM(''));
		...
		INSERT INTO NBS_ODSE.DBO.PUBLICHEALTHCASEFACT (
			...
			,NOTIFCURRENTSTATE
			,HOSPITALIZED_IND
			FROM NBS_ODSE.DBO.TEMP_PHC_FACT WITH (NOLOCK)
			);
