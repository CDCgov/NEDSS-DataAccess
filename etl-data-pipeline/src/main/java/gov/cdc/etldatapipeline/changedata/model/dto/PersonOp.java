...
public class PersonOp extends DebeziumMetadata {
    ...
    private String auth;
    @Column(name = "PERSON_BIRTH_COUNTRY")
    ...
    public PersonOp processPatient() {
        try {
            ...
            if (!ObjectUtils.isEmpty(telephone)) {
                Function<String, PersonOp> patientPhoneFn =
                        (useCd) -> Arrays.stream(UtilHelper.getInstance().deserializePayload(telephone,
                                        Phone[].class))
