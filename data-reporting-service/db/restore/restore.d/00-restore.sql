use master
go
-- Following Script will restore the DATABASE FROM "/var/opt/db-restore" Backups folder
...
DISK = N'/var/opt/db-restore/restore.d/NBS_ODSE.bak'
WITH  FILE = 1,
...
GO
RESTORE DATABASE [NBS_MSGOUTE] FROM  DISK = N'/var/opt/db-restore/restore.d/NBS_MSGOUTE.bak' WITH  FILE = 1,
...
NOUNLOAD,  STATS = 10
GO
RESTORE DATABASE [RDB] FROM  DISK = N'/var/opt/db-restore/restore.d/RDB.bak' WITH  FILE = 1,
MOVE N'RDB' TO N'/var/opt/mssql/data/rdb_data.mdf',
MOVE N'RDB_log' TO N'/var/opt/mssql/data/rdb_log.ldf',  NOUNLOAD,  STATS = 10
GO

---Step 2:
-- SQL Server 2005 Compatible Mode Scripts

USE master;
EXEC dbo.sp_dbcmptlevel @dbname=N'NBS_ODSE', @new_cmptlevel=90
GO
EXEC dbo.sp_dbcmptlevel @dbname=N'NBS_MSGOUTE', @new_cmptlevel=90
GO
EXEC dbo.sp_dbcmptlevel @dbname=N'NBS_SRTE', @new_cmptlevel=90
GO
EXEC dbo.sp_dbcmptlevel @dbname=N'RDB', @new_cmptlevel=90
GO
