                        ...
                        (
                            ...
                            FROM
                                Entity_locator_participation elp WITH (NOLOCK)
                                    ...
                                    JOIN NBS_SRTE.dbo.STATE_CODE sc with (NOLOCK) ON sc.STATE_CD=pl.STATE_CD
                                    JOIN NBS_SRTE.dbo.STATE_COUNTY_CODE_VALUE scc with (NOLOCK) ON scc.CODE=pl.CNTY_CD
                                    JOIN NBS_SRTE.dbo.COUNTRY_CODE cc with (nolock) ON cc.CODE=pl.CNTRY_CD
                            WHERE
                ...
                (
                    SELECT
                        (
                            ...
                            FROM
                                NBS_ODSE.dbo.Auth_user au WITH (NOLOCK)
                            WHERE
                                    p.ADD_USER_ID =  au.NEDSS_ENTRY_ID
                            FOR json path,
                                INCLUDE_NULL_VALUES
                        ) AS authadd
                ) AS authadd,
                    ...
                    SELECT
                        (
                            ...
                            FROM
                                NBS_ODSE.dbo.Auth_user au WITH (NOLOCK)
                            WHERE
                                    p.LAST_CHG_USER_ID = au.NEDSS_ENTRY_ID
                            FOR json path,
                                INCLUDE_NULL_VALUES
                        ) AS authchg
                ) AS authchg
