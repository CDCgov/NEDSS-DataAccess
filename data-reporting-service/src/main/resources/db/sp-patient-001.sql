...
SELECT
    ...
    case when (age_reported_unit_cd is not null or age_reported_unit_cd != '') then (select * from dbo.fn_get_value_by_cd_ques(p.age_reported_unit_cd, 'DEM218'))
    ...
    p.cd,
    p.curr_sex_cd,
    case when (p.curr_sex_cd is not null or p.curr_sex_cd != '') then (select * from dbo.fn_get_value_by_cd_ques(p.curr_sex_cd, 'DEM113'))
        end as current_sex,
    p.deceased_ind_cd,
    case when (p.deceased_ind_cd is not null or p.deceased_ind_cd != '') then (select * from dbo.fn_get_value_by_cd_ques(p.deceased_ind_cd, 'DEM127'))
        end as deceased_indicator,
    p.electronic_ind,
    p.ethnic_group_ind,
    case when (p.ethnic_group_ind is not null or p.ethnic_group_ind != '') then (select * from dbo.fn_get_value_by_cd_ques(p.ethnic_group_ind, 'DEM155'))
        end as ethnicity,
    p.birth_gender_cd,
    case when (p.birth_gender_cd is not null or p.birth_gender_cd != '') then (select * from dbo.fn_get_value_by_cd_ques(p.birth_gender_cd, 'DEM114'))
        end as birth_sex,
    p.deceased_time,
    p.last_chg_time,
    p.marital_status_cd,
    case when (p.marital_status_cd is not null or p.marital_status_cd != '') then (select * from dbo.fn_get_value_by_cd_ques(p.marital_status_cd, 'DEM140'))
    ...
    p.dedup_match_ind,
    p.speaks_english_cd,
    case when (p.speaks_english_cd is not null or p.speaks_english_cd != '') then (select * from dbo.fn_get_value_by_cd_ques(p.speaks_english_cd, 'NBS214'))
        end as speaks_english,
    p.ethnic_unk_reason_cd,
    case when (p.ethnic_unk_reason_cd is not null or p.ethnic_unk_reason_cd != '') then (select * from dbo.fn_get_value_by_cd_ques(p.ethnic_unk_reason_cd, 'NBS273'))
        end as unk_ethnic_rsn,
    p.sex_unk_reason_cd,
    case when (p.sex_unk_reason_cd is not null or p.sex_unk_reason_cd != '') then (select * from dbo.fn_get_value_by_cd_ques(p.sex_unk_reason_cd, 'NBS272'))
        end as curr_sex_unk_rsn,
    p.preferred_gender_cd,
    case when (p.preferred_gender_cd is not null or p.preferred_gender_cd != '') then (select * from dbo.fn_get_value_by_cvg(p.preferred_gender_cd, 'NBS_STD_GENDER_PARPT'))
        end as preferred_gender,
    p.additional_gender_cd,
    p.occupation_cd,
    case when (p.occupation_cd is not null or p.occupation_cd != '') then (select * from dbo.fn_get_value_by_cd_ques(p.occupation_cd, 'DEM139'))
        end as primary_occupation,
    p.prim_lang_cd,
    case when (p.prim_lang_cd is not null or p.prim_lang_cd != '') then (select * from dbo.fn_get_value_by_cd_ques(p.prim_lang_cd, 'DEM142'))
    ...
    p.add_user_id,
    case when p.add_user_id > 0 then (select * from dbo.fn_get_user_name(p.add_user_id))
        end as add_user_name,
    p.last_chg_user_id ,
    case when p.last_chg_user_id > 0 then (select * from dbo.fn_get_user_name(p.last_chg_user_id))
...
FROM
    nbs_odse.dbo.Person p WITH (NOLOCK)
  OUTER apply (
    SELECT
      *
    FROM
      ...
      (
        SELECT
          (
            ...
            FROM
              nbs_odse.dbo.Entity_locator_participation elp WITH (NOLOCK)
      ...
      (
        SELECT
          (
            SELECT
              tl.tele_locator_uid AS [email_tl_uid],
   ...
   WHERE
      ...
      (
        SELECT
          (
           SELECT
              pn.person_uid AS [pn_person_uid],
              ...
              case when (pn.nm_suffix is not null or pn.nm_suffix != '') then (select * from dbo.fn_get_value_by_cd_ques(pn.nm_suffix, 'DEM107'))
  			  end as name_suffix,
              pn.nm_degree AS [nmDegree],
              pn.person_name_seq AS [pn_person_name_seq],
              pn.last_chg_time AS [pn_last_chg_time]
            FROM
              nbs_odse.dbo.person_name pn WITH (NOLOCK)
            WHERE
      ...
      (
        SELECT
          (
            SELECT
              pr.person_uid AS [pr_person_uid],
...
WHERE
    p.person_uid in (SELECT value FROM STRING_SPLIT(@user_id_list, ','))
  AND p.cd = 'PAT'
