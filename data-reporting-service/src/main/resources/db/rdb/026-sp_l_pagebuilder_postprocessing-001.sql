...
if @debug = 'true' select * from #LOOKUP_TABLE1_INV_CAT;
...
IF @debug ='true' EXEC ('select * from '+@LOOKUP_N_CAT_TABLE_NAME+';');
...
BEGIN TRANSACTION;
		...
		*/
		...
		SET @exec_sql = 'DELETE LINC
			FROM '+QUOTENAME(@LOOKUP_TABLE_INC_TABLE_NAME)+' AS LINC
			JOIN '+QUOTENAME(@L_CAT_TABLE_NAME)+' AS L  ON LINC.PAGE_CASE_UID = L.PAGE_CASE_UID
			WHERE L.D_'+@category+'_KEY = LINC.D_'+@category+'_KEY;'
		...
		SET @exec_sql = ' INSERT INTO '+QUOTENAME(@LOOKUP_TABLE_INC_TABLE_NAME)+'
		SELECT ltn.PAGE_CASE_UID AS PAGE_CASE_UID_N, lt1.PAGE_CASE_UID AS PAGE_CASE_UID_NE, ltn.D_'+@category+'_KEY  AS D_'+@category+'_KEY_N,
...
EXEC ('UPDATE '+@LOOKUP_TABLE_INC_TABLE_NAME+'
		...
		IF @debug ='true' PRINT @LOOKUP_TABLE_INC_TABLE_NAME;
