...
(procedure_name,
 param_id_list,
 status)
Values ('sp_nrt_patient_postprocessing',
        @id_list,
        'START');
set @log_id = @@IDENTITY;
...
where nrt.patient_uid in (SELECT value FROM STRING_SPLIT(@id_list, ','));
...
FROM #temp_patient_table tpt
         left join dbo.d_patient p on tpt.patient_key = p.patient_key
         left join
     (select ROW_NUMBER() over (order by tpt.patient_uid) as row_num, patient_uid
      from #temp_patient_table tpt
      where tpt.patient_key is null) as rn on rn.patient_uid = tpt.patient_uid
